...
public partial class Main : ContentPage
{
    ...
    {
        var resultado = ConsumeAPI.Crud<Clasificacion>.Create(ApiUrl, new Clasificacion
        {
            Id = 0,
    ...
    {
        var resultado = ConsumeAPI.Crud<Clasificacion>.Read_ById(ApiUrl, int.Parse(txtId.Text));
        if (resultado != null)
        {
            txtId.Text = resultado.Id.ToString();
    ...
    {
        ConsumeAPI.Crud<Clasificacion>.Update(ApiUrl, int.Parse(txtId.Text), new Clasificacion
        {
    ...
    {
        ConsumeAPI.Crud<Clasificacion>.Delete(ApiUrl, int.Parse(txtId.Text));
    ...
    {
        ConsumeAPI.Crud<Producto>.Delete(ApiUrlProd, int.Parse(txtIdProducto.Text));
        txtIdProducto.Text = "";
        txtIdProducto.Text = "";
    ...
    {
        ConsumeAPI.Crud<Producto>.Update(ApiUrlProd, int.Parse(txtIdProducto.Text), new Producto
        {
    ...
    {
        var prod = ConsumeAPI.Crud<Producto>.Read_ById(ApiUrlProd, int.Parse(txtIdProducto.Text));
        if (prod != null)
        {
    ...
    {
        var prod = ConsumeAPI.Crud<Producto>.Create(ApiUrlProd, new Producto
        {
            Id = 0,
            Nombre = txtNombreProducto.Text,
            Existencia = double.Parse(txtExistencia.Text),
